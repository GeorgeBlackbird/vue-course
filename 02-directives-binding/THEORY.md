...
```
  ...
  Если `isFormInvalid` станет `false`, кнопка станет активной.
...
*(Здесь был бы полезен GIF, показывающий, как элемент меняет свой вид при изменении значений `isActive` или `hasError`)*
