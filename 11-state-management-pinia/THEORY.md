# Тема 11: 🗃️ Управление состоянием: Pinia

Привет! По мере того как мы строили наше приложение, ты мог заметить, что управлять данными становится все сложнее. У нас есть состояние корзины (`cartItems`) в `App.vue`, а добавляем мы в нее товары из компонента `BookCard`, который находится внутри `HomePage.vue`. Чтобы это работало, нам приходилось "пробрасывать" данные и события через несколько уровней компонентов. Это не всегда удобно.
...
* **Pinia - Официальная документация (Начало работы):** https://pinia.vuejs.org/getting-started.html
